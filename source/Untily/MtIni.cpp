...
bool MtIniA::SaveFile(const char* szFilePath/* = nullptr*/)
{
    if (!szFilePath)
    {
        if (!m_bOpen)
...
void MtIniA::Serial(const char* szKey, int& nVal, bool bSerial)
{
    if (bSerial)
    {
        SetInt(szKey, nVal);
    ...
    {
        if (m_pSimpleIni->GetValue(m_szSection, szKey, nullptr))
        {
            nVal = m_pSimpleIni->GetLongValue(m_szSection, szKey, 0);
        }
    }
}

void MtIniA::Serial(const char* szKey, bool& nVal, bool bSerial)
{
    if (bSerial)
    {
        SetInt(szKey, nVal);
...
void MtIniA::Serial(const char* szKey, double& nVal, bool bSerial)
{
    if (bSerial)
    {
        SetDouble(szKey, nVal);
    }
    else
    {
        const char* pszVal = m_pSimpleIni->GetValue(m_szSection, szKey, nullptr);
        if (pszVal)
        {
            if (pszVal && (pszVal[0] != 0))
            {
                double dVal = atof(pszVal);
                nVal = dVal;
            }
            else
            {
...
void MtIniA::Serial(const char* szKey, std::string& strVal, bool bSerial)
{
    if (bSerial)
    {
    ...
    {
        strVal = GetString(szKey, "");
    }
}

#ifdef _AFX
void MtIniA::Serial(const char* szKey, CStringA& strVal, bool bSerial)
{
    if (bSerial)
    {
        SetString(szKey, (LPCSTR)strVal);
    }
    else
    {
        strVal = GetString(szKey, "");
...
void MtIniA::Serial(const char* szKey, std::vector<int>& nVecIntVal, bool bSerial)
{
    if (bSerial)
    {
        DelKeyName(szKey);
...
bool MtIniW::SaveFile(const wchar_t* szFilePath/* = nullptr*/)
{
    if (!szFilePath)
    {
        if (!m_bOpen)
...
void MtIniW::Serial(const wchar_t* szKey, int& nVal, bool bSerial)
{
    if (bSerial)
    {
        SetInt(szKey, nVal);
...
void MtIniW::Serial(const wchar_t* szKey, bool& nVal, bool bSerial)
{
    if (bSerial)
    {
        SetInt(szKey, nVal);
...
void MtIniW::Serial(const wchar_t* szKey, double& nVal, bool bSerial)
{
    if (bSerial)
    {
        SetDouble(szKey, nVal);
    ...
    else
    {
        ...
        const wchar_t* pszVal = m_pSimpleIni->GetValue(m_szSection, szKey, nullptr);
        if (pszVal)
        {
            if (pszVal && (pszVal[0] != 0))
            {
                double dVal = _wtof(pszVal);
                nVal = dVal;
            }
            else
            {
...
void MtIniW::Serial(const wchar_t* szKey, std::wstring& strVal, bool bSerial)
{
    if (bSerial)
    {
        SetString(szKey, strVal.c_str());
    ...
    {
        strVal = GetString(szKey, L"");
    }
}

#ifdef _AFX
void MtIniW::Serial(const wchar_t* szKey, CStringW& strVal, bool bSerial)
{
    if (bSerial)
    {
        SetString(szKey, (LPCWSTR)strVal);
    }
    else
    {
        strVal = GetString(szKey, L"");
...
void MtIniW::Serial(const wchar_t* szKey, std::vector<int>& nVecIntVal, bool bSerial)
{
    if (bSerial)
    {
        DelKeyName(szKey);
